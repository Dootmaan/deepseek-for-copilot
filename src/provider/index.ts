...
/**
 * DeepSeek Chat Provider — implements `vscode.LanguageModelChatProvider` so DeepSeek
...
export class DeepSeekChatProvider implements vscode.LanguageModelChatProvider {
	 ...
	 *
	 * DeepSeek's tokenizer rules ~0.3 token/English-char and ~0.6 token/Chinese-char;
	 * 4.0 is a reasonable starting estimate that converges quickly with usage data.
	 */
		...
		context.subscriptions.push(
			...
			vscode.workspace.onDidChangeConfiguration((e) => {
				if (
					...
					e.affectsConfiguration('deepseek-copilot.customModels')
				) {
