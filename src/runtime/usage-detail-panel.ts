...
/**
 * Singleton webview panel showing DeepSeek balance detail. Clicking the DeepSeek Usage
...
export class UsageDetailPanel {
	...
	private constructor(
		...
		this.themeSub = vscode.window.onDidChangeActiveColorTheme(() =>
			this.render(this.bar.getSnapshot()),
		);
	...
	private render(message: UsagePanelMessage | null): void {
		if (
			...
			this.lastMessage.status !== 'loading'
		) {
			return;
		}
...
/** Render the HTML body for the `ok` status: balance rows + footer. */
function renderOkBody(msg: UsagePanelMessage): string {
	...
	if (msg.lastUpdated !== undefined) {
		lines.push(
			`<div class="last-updated">${escapeHtml(
				s.lastUpdated.replace('{0}', new Date(msg.lastUpdated).toLocaleTimeString()),
			)}</div>`,
		);
	}
	if (msg.offline) {
...
/** Render the balance section: one block per currency (CNY, USD, …) with rows. */
function renderBalanceBody(msg: UsagePanelMessage): string {
	...
	if (b.isAvailable === false) {
		lines.push(`<div class="insufficient">${escapeHtml(useInsufficientNote(msg))}</div>`);
	}
	for (const row of b.rows) {
		const cur = escapeHtml(row.currency);
		lines.push(`<div class="currency-block"><h3>${cur}</h3>`);
		if (row.available !== undefined) {
			lines.push(
				`<div class="balance-row primary"><span>${escapeHtml(s.balanceAvailable)}</span><span class="balance-value">${cur} ${escapeHtml(row.available)}</span></div>`,
			);
		}
		if (row.total !== undefined && row.total !== row.available) {
			lines.push(
				`<div class="balance-row"><span>${escapeHtml(s.balanceTotal)}</span><span class="balance-value">${cur} ${escapeHtml(row.total)}</span></div>`,
			);
		}
		if (row.granted !== undefined && row.granted !== '0') {
			lines.push(
				`<div class="balance-row"><span>${escapeHtml(s.balanceGranted)}</span><span class="balance-value">${cur} ${escapeHtml(row.granted)}</span></div>`,
			);
		}
		if (row.toppedUp !== undefined && row.toppedUp !== '0') {
			lines.push(
				`<div class="balance-row"><span>${escapeHtml(s.balanceToppedUp)}</span><span class="balance-value">${cur} ${escapeHtml(row.toppedUp)}</span></div>`,
			);
		}
		lines.push('</div>');
	}
...
function renderStatusBody(msg: UsagePanelMessage): string {
	...
	const text = s.status[msg.status] || s.status['no-data'];
...
function themeCss(theme: 'dark' | 'light'): string {
	...
	return `
		...
		.balance-section { margin-top: 12px; }
		.balance-section h2 { font-size: 1rem; margin-bottom: 12px; color: ${fg}; }
		.currency-block { margin-bottom: 16px; padding: 10px 12px; border: 1px solid ${border}; border-radius: 4px; }
		.currency-block h3 { margin: 0 0 8px 0; font-size: 0.85rem; color: ${muted}; text-transform: uppercase; letter-spacing: 0.05em; }
		.balance-row { display: flex; justify-content: space-between; padding: 4px 0; font-size: 0.9rem; }
		.balance-row.primary { font-size: 1.05rem; font-weight: 600; }
		...
		.insufficient { color: ${warn}; }
	`;
}

/** Resolve the "balance insufficient" note from i18n (mirrors the tooltip). */
function useInsufficientNote(_msg: UsagePanelMessage): string {
	return t('usage.tooltip.insufficient');
}
