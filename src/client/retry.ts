 ...
 *
 * Non-retryable HTTP responses and exhausted retries throw a `DeepSeekRequestError`
...
export async function fetchChatCompletionWithRetry(
	...
	for (let attempt = 0; ; attempt++) {
		...
		logger.warn(
			`DeepSeek API returned HTTP ${response.status} on attempt ${attempt + 1}/${maxAttempts}; retrying in ${delay}ms`,
		);
